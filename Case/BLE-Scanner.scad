...
case_length = pcb_length + 2 * case_thickness + slackness_pcb + 0 * case_edge_radius;
case_width = pcb_width + 2 * case_thickness + slackness_pcb + 0 * case_edge_radius;
...
snap_offset = case_thickness - 2 * snap_height;
...
// LED
led_whole_length = 5.0;
led_whole_width = 2.0;
led_whole_radius = 0.8;
led_offset_x = pcb_length - led_whole_length - 1;
led_offset_y = 2;
...
module make_case_lower_shell()
{
    difference() {
        ...
        translate([-case_thickness / 2,case_width / 2 - usbport_width / 2 - slackness_pcb,case_thickness + usbport_offset_z])
            rotate([90,0,90])
        ...
        for (side = [-1,+1])
            translate([(case_length - case_snap_width) / 2,case_width / 2 + side * (case_width / 2 - case_thickness),case_lower_height - snap_height - snap_offset])
                rotate([90 * -side,0,0])
...
module make_case_upper_shell()
{
    difference() {
        ...
        translate([case_thickness + led_offset_x,case_thickness + led_offset_y,-case_thickness / 2])
            make_rounded_box(case_thickness * 2,led_whole_length,led_whole_width,led_whole_radius);
    ...
    for (side = [-1,+1])
        union() {
            ...
            translate([
                    ...
                    case_thickness])
                translate([0,0* -snap_height,0])
                    make_rounded_box(case_thickness * 3/2,case_length - 3 * 2 * case_edge_radius,case_thickness,snap_height / 2);

            // ... and add the snap
            translate([
                    (case_length - case_snap_width * 1.1) / 2,
                    case_width / 2 + side * (case_width / 2 - case_thickness - slackness_snap),
                    case_thickness + snap_height + snap_offset])
                rotate([90 * -side,0,0])
                    translate([0,-snap_height,0])
